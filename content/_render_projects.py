#!/usr/bin/env python3
"""Render the projects-45 carousel (How to set up Projects in Claude).
...
Substitutes the base64 Ultron logo for __LOGO_URI__ at export, then scrubs metadata.

Usage: python3 content/_render_projects.py
"""
import os, base64, subprocess, tempfile, img2pdf
from playwright.sync_api import sync_playwright

HERE = os.path.dirname(os.path.abspath(__file__))
HTML = os.path.join(HERE, 'projects-45-tiktok-carousel.html')
LOGO = os.path.join(HERE, 'ultron-logo.png')
LI   = os.path.join(HERE, 'projects-45-carousel-{:02d}.png')         # LinkedIn name-set
TT   = os.path.join(HERE, 'projects-45-tiktok-carousel-{:02d}.png')  # TikTok name-set
PDF  = os.path.join(HERE, 'projects-45-carousel.pdf')
