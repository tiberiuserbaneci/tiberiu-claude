...
def progress(d,page,n):
    ...
    f=B.mono(26); lbl=f"{page:02d} / {n:02d}"; lw=d.textlength(lbl,font=f); d.text((W-MX-lw,y-46),lbl,font=f,fill=MUTED)

def footer(base,page,n):
    d=ImageDraw.Draw(base); lg=Image.open(ULOGO).convert("RGBA"); lg.thumbnail((52,52),Image.LANCZOS)
    base.alpha_composite(lg,(MX,1756)); f=B.mono(30); d.text((MX+66,1766),"51ultron.com",font=f,fill=CORAL)
    f2=B.mono(26); lbl=f"{page:02d} / {n:02d}"; lw=d.textlength(lbl,font=f2); d.text((W-MX-lw,1770),lbl,font=f2,fill=MUTED)

def swipe(base):
    bw,bh=252,78; bx=(W-bw)//2; by=1726
