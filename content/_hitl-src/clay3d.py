...
.strike{{text-decoration:line-through;text-decoration-color:rgba({acc},.85);text-decoration-thickness:2.5px}}
...
def panels(acc):
 ...
 "warmup":f"""
  ...
  <div style="display:flex;align-items:baseline;gap:12px;margin-bottom:18px">
    <span style="font-weight:900;font-size:64px;color:{A};text-shadow:0 6px 18px rgba({acc},.35)">99.2%</span>
    <span style="font-weight:700;font-size:16px;color:#8f8f85">inbox placement</span></div>
  <div style="display:grid;gap:13px">
    <div><div style="display:flex;justify-content:space-between;margin-bottom:6px"><span class="rn" style="font-size:14.5px">Inbox</span><span class="rs">99.2%</span></div><div class="bar"><i style="--w:99%"></i></div></div>
    <div><div style="display:flex;justify-content:space-between;margin-bottom:6px"><span class="rn" style="font-size:14.5px">Spam</span><span class="rs">0.3%</span></div><div class="bar"><i style="--w:3%"></i></div></div>
    <div><div style="display:flex;justify-content:space-between;margin-bottom:6px"><span class="rn" style="font-size:14.5px">Bounce</span><span class="rs">0.3%</span></div><div class="bar"><i style="--w:3%"></i></div></div>
  </div>
 """,
 "seowriter":f"""
...
if __name__=="__main__":
    ...
    with sync_playwright() as pw:
        ...
        for s in stems:
            html=f"<!DOCTYPE html><html><head><meta charset='utf-8'><style>{css(acc)}</style></head><body><div class='panel' id='panel'>{P[s]}</div></body></html>"
