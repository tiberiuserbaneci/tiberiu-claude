...
T2.CONTENT=[
  ("LIVE", [[wo("A whole team's work,")],[co("running live.")]], f"{PH}/p2_deals.png", 1.0),
  ("BUILD", [[wo("It builds and")],[co("ships itself.")]], f"{PH}/p2_build.png", 1.0),
  ("UNITS", [[wo("Seven units.")],[co("One operator.")]], f"{PH}/p2_units.png", 1.0),
...
def _body(base, eyebrow, head, objpath, page, n, last=False, fill=1.0):
    ...
    if last:
        ...
        return
    T2.ls_text(d,(T2.MX,302),eyebrow,T2.mono(26),T2.CORAL,4)
    ...
    T2.place_in_zone(base, T2.crop_obj(Image.open(objpath)), (130,512,950,1566), fill=1.0)
...
if __name__=="__main__":
    a=T2.deck_poll(f"{T2.OUTBASE}/phone_tt",False); b=T2.deck_poll(f"{T2.OUTBASE}/phone_ig",True)
