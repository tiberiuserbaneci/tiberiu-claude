...
def panels_for(slug,acc):
 ...
 "sellonce":{
  "listtrap": f_folder("ideas.md","OPENED WEEKLY, SHIPPED NEVER",[row("50 product ideas","curated, highlighted",chip="saved"),row("Products live","the honest count",chip="0"),row("The feeling","progress, falsely",chip="trap")]),
  "pick9": f_paper("The pick","ALREADY VALIDATED","The thing you get asked about weekly",'<div style="display:flex;align-items:center;gap:12px"><span class="dot"></span><div><div class="rn" style="color:#17150F">The template you explain</div><div class="rs" style="color:#8a7a5e">demand is already visible</div></div><span class="chip">That one</span></div>'),
  "build9": f_env("THE BUILD","One sentence in.","Your workflow becomes the pack, the guide, the system: from your own files.","An evening"),
  "page9": f_browser("your-product.com &middot; live same day",hd("The page","NO DESIGNER, NO DELAY")+row("Hero + pricing + FAQ","from the component pack",chip="live")+row("Your tokens","exactly, everywhere",chip="live")+row("Checkout","wired to delivery",chip="live")),
  "delivery": f_metro("03:14","THE NIGHT SALE",[("Paid","while you slept",1),("Delivered","same minute",1),("Receipt + follow-up","sent, gated rules",1),("You","found out at 07:00",0)]),
  "loop9": f_chartline("Version two","FEEDBACK COMPOUNDS",[30,42,50,62,74,86,100],"every buyer question funnels into the next update, drafted for your tap"),
  "spread": f_shelf("The library","ONE FUNDS THE NEXT",[("Product one","validated the niche","live"),("Product two","cloned the pipeline","live"),("Product three","same desk, new door","next")]),
  "math9": f_stamp("The math","CREATED ONCE","the shelf never closes",[("Inventory","none"),("Shipping","none"),("Sales at 03:14","yes")],"SHIPPED"),
 },
 "starthere":{
  "lists9": f_rows("","The 45 doors","NEVER LOCKED",[row("Freelance platforms","open for years",chip="open"),row("Store builders","open for years",chip="open"),row("Writing platforms","open for years",chip="open"),row("Your first dollar","still waiting",strike=True,chip="why")]),
  "gap9": f_split("SIGNUPS",["Make the account","Browse the feed","Wait for luck"],"SYSTEMS",["Show up daily","Reply in minutes","Follow up, always"]),
  "profile9": f_paper("The profile","AN OFFER PAGE","Not a resume",'<div style="display:flex;align-items:center;gap:12px"><span class="dot"></span><div><div class="rn" style="color:#17150F">Outcome, proof, next step</div><div class="rs" style="color:#8a7a5e">drafted by the desk</div></div><span class="chip">Rewrite</span></div>'),
  "response9": f_dial(82,"FIRST REPLY","min","from brief to answer","read, drafted, parked for your tap: speed is the algorithm"),
  "follow9": f_pills([row("Quiet client","chased on the trigger",chip="day 3"),row("Delivered work","review requested",chip="day 1"),row("Old client","re-pitched with proof",chip="day 30")]),
  "portfolio9": f_fan("Every delivery","BECOMES PUBLIC PROOF",[row("The case note","drafted from the work",chip="auto"),row("The post","in your voice",chip="auto"),row("The pitch asset","reused forever",chip="auto")]),
  "onedoor": f_podium([("BROWSE","45 tabs",0),("SIGNUP","again",0),("DABBLE","weekly",0),("ONE DOOR","30 days",1),("THE LOOP","daily",1)]),
  "truth9": f_keys([("THE LIST","WAS ALWAYS FREE",False),("THE DISCIPLINE","IS THE PRICE",True)]),
 },
 "fivetools":{
  "door1": f_bubble("Quick questions, summaries, first drafts.","Chat","THE LOBBY","most people stop here",A),
  "door2": f_folder("Projects","THE MEMORY DOOR",[row("Monthly reporting","context that survives",chip="door 2"),row("Client work","never re-explained",chip="door 2"),row("Board prep","picks up where you left",chip="door 2")]),
  "door3": f_browser("your files &middot; worked, not discussed",hd("Cowork","THE HANDS")+row("Folders","organized in place",chip="door 3")+row("Spreadsheets","read and written",chip="door 3")+row("Reports","drafted where they live",chip="door 3")),
  "door4": f_pills([row("/brief account","same play, every run",chip="door 4"),row("/audit funnel","no re-prompting",chip="door 4"),row("/draft offer","your rules baked in",chip="door 4")]),
  "door5": f_console("Code","THE BUILDER DOOR",[("Pages + dashboards","from plain English",True),("Tested","before you see them",True),("Shipped","on your domain",True)],buttons=False),
  "mistake9": f_receipt("THE DOOR-ONE TAX","PAID WEEKLY",[("Same workflow","re-typed into Chat"),("Same context","re-pasted"),("Same output","half as good")],"A Skills job","done badly"),
  "desk9": f_rings("THE ROUTER",[("You type","the goal, plain"),("It opens","the right door"),("Your memory","travels through all five")],"no door knowledge required"),
  "unlock9": f_stamp("The unlock","80% OF MASTERY","is knowing the door exists",[("Door two","tonight"),("Door four","this week"),("Door five","when it pays")],"OPEN UP"),
 },
 "direction":{
  "mist10": f_split("PROMPTERS",["Ask and hope","No reader, no goal","Accept draft one"],"DIRECTORS",["Brief the audience","Attach the bar","Pick from options"]),
  "dir1": f_env("DIRECTION 1","Give it the reader.","Founders scanning at 1.7 seconds is a brief. A topic is a shrug.","Briefed"),
  "dir2": f_paper("Direction 2","TONE IS DATA","Five real posts in, voice locked",'<div style="display:flex;align-items:center;gap:12px"><span class="dot"></span><div><div class="rn" style="color:#17150F">Not adjectives</div><div class="rs" style="color:#8a7a5e">samples, measured and matched</div></div><span class="chip">Locked</span></div>'),
  "dir3": f_fan("Direction 3","HOOKS IN THREE SHAPES",[row("The confession","I quit nine tools...",chip="A"),row("The number","800 vs 47,000...",chip="B"),row("The enemy","the gurus sell you...",chip="C")]),
  "dir4": f_shelf("Direction 4","ONE IDEA, FIVE FORMATS",[("Post + carousel","same spine, two shapes","out"),("Reel script + caption","native pair","out"),("Newsletter","the long form","out")]),
  "dir5": f_gauge(92,"THE BAR","set before the draft","your scoring rules ride in the instructions: ranked before you look"),
  "burnout": f_calendar("The streak","HELD BY THE DESK",[("M","POST"),("T","POST"),("W","POST"),("T","POST"),("F","POST"),("S",None),("S",None),("8","POST"),("9","POST"),("10","POST"),("11","POST"),("12","POST"),("13",None),("14","POST")],"consistency is a system, not a mood"),
  "shift9": f_keys([("PROMPT HARDER","THE AMATEUR MOVE",False),("DIRECT BETTER","THE OPERATOR MOVE",True)]),
 },
 "skipcourse":{
  "path9": f_rows("","The curriculum","THREE COURSES DEEP",[row("Prompting course","six hours of anatomy",strike=True,chip="skip"),row("Automation course","toy examples",strike=True,chip="skip"),row("Coding course","for a job you do not want",strike=True,chip="skip"),row("Your pipeline","the only real classroom",chip="enroll")]),
  "flip9": f_bubble("Every lesson that matters comes from a real run on real work.","The flip","TUITION IN CENTS","run job one",A),
  "run1": f_odo(["2","0"],0,"ACCOUNTS BRIEFED, RUN ONE","teaches more than any prompt anatomy lecture &middot; lesson one: context"),
  "run10": f_folder("Run ten","THE RULES EXIST NOW",[row("no-list.md","born from corrections",chip="growing"),row("style.md","the fixes that stuck",chip="growing"),row("The drafts","stopped disappointing",chip="felt")]),
  "run50": f_console("Run fifty","YOU STOPPED INITIATING",[("Jobs fire","on their own triggers",True),("You approve","from the digest",True),("The desk","runs the day",True)],buttons=False),
  "run100": f_badge(PLANE,"1","Run 100: your workflow, minted.","the coding course became one command you own"),
  "gate9": f_ticket("The classroom","SAFE TO FAIL",tick_cells([("RUN","Try",1),("FAIL","Inside",1),("FIX","Once",1),("KEEP","Rule",1),("SHIP","Gated",0)]),"MISTAKES STAY INTERNAL","THE CLASSROOM HAS A KEEL"),
  "diploma": f_chartline("The diploma","A HUNDRED RUNS IN",[20,30,42,54,66,80,92,100],"no certificate: a pipeline that moves and a system that remembers"),
 },

 "eyes":{
  "jar": f_rings("BLIND",[("Your market","unseen"),("Your inbox","unseen"),("This morning","unseen")],"smart and sightless: it answers from memory, not from now"),
  "eyes10": f_browser("agent search &middot; structured, not blue links",hd("The eyes","BUILT FOR AGENTS")+row("Query in","clean data out",chip="wired")+row("Extract + crawl","site to signal",chip="wired")+row("Under a minute","to connect",chip="wired")),
  "crawl": f_rows("","The crawl","THE WEB, AS DATA",[row("Competitor pages","read nightly",chip="fed"),row("Pricing changes","diffed on sight",chip="fed"),row("Job boards","hiring signals mined",chip="fed"),row("All of it","lands in briefs",chip="fed")]),
  "watch": f_console("The watch","TRIGGERS, NOT TABS",[("Rival launches","alert on change",True),("Market moves","scored to your ICP",True),("Mention spikes","digest-bound",True)],buttons=False),
  "proof10": f_odo(["1",",","2","8","4"],3,"COMPANIES READ LIVE","hiring, funding, stack changes &middot; this morning, not last year"),
  "diff10": f_split("BLIND AI",["Answers from memory","Improvises the gaps","Sounds confident"],"SIGHTED AI",["Cites the source","Quotes the date","Earns the trust"]),
  "setup10": f_pills([row("Search","wired, gated",chip="sense"),row("Crawl","wired, gated",chip="sense"),row("Watch","wired, gated",chip="sense")]),
  "bar10": f_bubble("Never trust an answer that cannot see today.","The test","ASK WHAT CHANGED THIS WEEK","wire the eyes",A),
 },
 "syllabus":{
  "mod1": f_env("MODULE 1 &middot; MEMORY","/init my business","The course teaches you to write config. The desk interviews you and writes its own.","Automated"),
  "mod2": f_shelf("Module 2","AGENT TEAMS, PREWIRED",[("CORTEX + SPECTER","research feeds outbound","composed"),("STRIKER + PULSE","deals feed content","composed"),("SENTINEL + gate","builds, then parks","composed")]),
  "mod3": f_pills([row("71 skills","working, not theoretical",chip="stocked"),row("Subagents","spawned per job",chip="stocked"),row("Handoffs","clean, automatic",chip="stocked")]),
  "mod4": f_keys([("THE LECTURE","SHOWS DEMOS",False),("THE DESK","BOOKS, FILLS, FILES",True)]),
  "mod5": f_vault("GATE","MODULE FIVE, BY DEFAULT"),
  "gap10": f_receipt("THE CONFIG HOURS","WHAT THEY COST",[("Watching module 2","an evening"),("Writing configs","a weekend"),("Debugging them","the next one")],"Hours selling","zero"),
  "hour10": f_metro("The onboarding","THE SYLLABUS, LIVED",[("Connect","the stack, two clicks",1),("Interview","once, ten minutes",1),("First run","twenty briefs",1),("By lunch","all five modules",0)]),
  "point10": f_stamp("The point","KNOWLEDGE IS NOT THE MOAT","the ones shipping are not studying",[("Install","today"),("Run + correct","daily"),("Repeat","weekly")],"OPERATORS SHIP"),
 },
 "fiveam":{
  "clockin": f_console("05:00","THE SHIFT STARTS ITSELF",[("Research","wakes on schedule",True),("Triage","wakes on schedule",True),("Drafting","wakes on schedule",True)],buttons=False),
  "trends": f_browser("overnight movers &middot; scored",hd("05:10","THE MARKET, READ")+row("Niche trends","ranked to your ICP",chip="scored")+row("Rival moves","flagged for the digest",chip="scored")+row("Before sunrise","all of it",chip="done")),
  "drafted": f_fan("05:30","TODAY'S SLOTS, FILLED",[row("Post + carousel","your voice, ranked",chip="parked"),row("Reel script","the boring rules",chip="parked"),row("Newsletter","inbox-ready",chip="parked")]),
  "builtflow": f_env("06:00","Yesterday's friction.","The job you cursed at 16:00 became a flow by 06:00.","Self-extended"),
  "digest10": f_paper("06:45","ONE PAGE, EVERYTHING","Done, parked, waiting, suggested",'<div style="display:flex;align-items:center;gap:12px"><span class="dot"></span><div><div class="rn" style="color:#17150F">Your company, one screen</div><div class="rs" style="color:#8a7a5e">reading time: four minutes</div></div><span class="chip">07:00</span></div>'),
  "walkin": f_podium([("GRIND","done",0),("SORT","done",0),("DRAFT","done",0),("APPROVE","you",1),("DECIDE","you",1)]),
  "cost10": f_dial(70,"THE NIGHT SHIFT","cents","no overtime, no burnout","payroll stayed flat while the shift grew"),
  "flip10": f_bubble("Stop starting your day. Start joining it.","The 5am team","EXISTS ALREADY","hire the night shift",A),
 },
 "citedweb":{
  "stale": f_battery(31,"HOW CURRENT ITS FACTS ARE","training data ends: your market does not &middot; unwired AI improvises the difference"),
  "standard": f_rows("","The standard","CITATIONS OR SILENCE",[row("Every figure","carries a link",chip="dated"),row("Every claim","carries a date",chip="dated"),row("Every brief","names its sources",chip="dated"),row("Anything else","does not ship",chip="rule")]),
  "brief10": f_paper("The brief","QUOTES, NOT MEMORIES","Yesterday's hiring page, cited",'<div style="display:flex;align-items:center;gap:12px"><span class="dot"></span><div><div class="rn" style="color:#17150F">Role, date, link</div><div class="rs" style="color:#8a7a5e">warm data closes calls</div></div><span class="chip">Sourced</span></div>'),
  "numbers10": f_receipt("MONTH-END","FETCHED, NOT ARGUED",[("Revenue","live, sourced"),("Pipeline","live, sourced"),("Spend","live, sourced")],"Meetings about sheets","zero"),
  "rivals": f_metro("The rivals","DATED AND LINKED",[("Price change","caught, cited",1),("New launch","in the digest",1),("Quiet quarter","also a signal",1),("Rumors","never shipped",0)]),
  "trust10": f_keys([("GUESSES","STAY ON YOUR DESK",False),("RECEIPTS","TRAVEL",True)]),
  "habit10": f_stamp("The habit","SET ONCE","ask for the source one time",[("The rule","cites forever"),("The reports","carry receipts"),("The trust","compounds")],"CITED"),
  "line10": f_gauge(12,"DATED","of what it asserts","undated knowledge is opinion wearing a suit: demand the date"),
 },
 "twentyplays":{
  "palette": f_pills([row("/write-blog","an SEO post, one command",chip="play"),row("/repurpose","one piece, eight platforms",chip="play"),row("/audit-rivals","gaps flagged",chip="play")]),
  "playblog": f_env("PLAY /BLOG","Keyword in.","Structured draft out, in your voice, ranked before review.","The factory"),
  "playrep": f_fan("Play /repurpose","ONE ARTICLE, EIGHT SHAPES",[row("Posts + tweets","native pairs",chip="out"),row("Carousel script","ten pages",chip="out"),row("Newsletter","the long form",chip="out")]),
  "playseq": f_metro("Play /sequence","WRITTEN TO STAGE",[("Cold","the door-opener",1),("Warm","the proof email",1),("Decision","the closer",1),("All parked","for your tap",0)]),
  "playaudit": f_split("THEY COVER",["Price, endlessly","Feature dumps","The same promise"],"LEFT OPEN",["The switching pain","The hidden cost","Your angle, waiting"]),
  "playhead": f_dial(84,"THE HEADLINE","10","scored before one ships","emotion, clarity, curiosity: graded against the boring rules"),
  "diff10b": f_podium([("FEATURE","a button",0),("TOOL","a tab",0),("PROMPT","a guess",0),("PLAY","a workflow",1),("PALETTE","the dept",1)]),
  "monday10": f_ticket("The Monday","BEFORE COFFEE",tick_cells([("PLAN","/cal",1),("DRAFT","/blog",1),("MULTIPLY","/rep",1),("AUDIT","/gap",1),("QUEUE","Tap",0)]),"FIVE COMMANDS, ONE HOUR","THE DEPARTMENT, TYPED"),
 },
    }[slug]
