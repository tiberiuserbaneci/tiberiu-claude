...
PANELS={
 ...
 "hubspot":"a sales pipeline board, the panel body DARK CHARCOAL like the reference: three upright columns side by side, each topped by a single one-word header label 'Qualified', 'Proposal', 'Won'; each column holds exactly ONE plain rounded deal card with only a soft avatar dot and one faint grey placeholder bar (no words on the cards); the card under 'Won' is filled warm orange with a small white check. The only visible words on the whole panel are the three column headers.",
