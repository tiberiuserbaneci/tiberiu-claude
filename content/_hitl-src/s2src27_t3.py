#!/usr/bin/env python3
# TIER 3 - THE OPERATOR STACK, rebuilt to the WIRE-ITS-EYES bar: each panel a UNIQUE hand-built
...
# 1. ROUTER - one job token routed down 3 tier lanes, SMART lane lit/picked (dark)
def router():
    ...
    return f'''<div style="width:900px;{CARD};padding:34px 40px 34px">
      {htitle("It picks the model, not you","MODEL ROUTER")}
      <div style="position:relative;height:460px">
        ...
        <div style="position:absolute;left:14px;top:206px;width:88px;font-family:DM Mono;font-size:12px;color:#c9c3b8;text-align:center">write 12<br>follow-ups</div>
        {cards}
      </div>
      {cap("one job in, the cheapest tier that can actually do it - cents, not dollars.")}</div>'''

# 2. RESEARCH - bezier flow: 4 raw sources converge into one ranked brief (dark)
def research():
    W,H=820,440
    src=[("18,400 profiles",90),("firmographics",190),("tech stack",290),("buying intent",390)]
    edges=""; nodes=""
    hubx,huby=650,240
    for nm,y in src:
        mx=(190+hubx)/2
        edges+=f'<path d="M200 {y} C{mx:.0f} {y},{mx:.0f} {huby},{hubx-72} {huby}" stroke="rgba(212,162,127,.5)" stroke-width="2.5" fill="none"/>'
        nodes+=f'''<rect x="40" y="{y-27}" width="160" height="54" rx="12" fill="#2a2724" stroke="rgba(255,255,255,.09)"/>
          <text x="120" y="{y+5}" text-anchor="middle" font-family="DM Mono" font-size="15" fill="#c9c3b8">{nm}</text>'''
    return f'''<div style="width:900px;{CARD};padding:38px 40px 34px">
      {htitle("The whole web, one ranked brief","CORTEX")}
      <svg width="{W}" height="{H}" viewBox="0 0 {W} {H}" style="display:block;margin:0 auto">
        ...
        <filter id="gh2" x="-80%" y="-80%" width="260%" height="260%"><feDropShadow dx="0" dy="0" stdDeviation="20" flood-color="rgb({ACC})" flood-opacity="0.5"/></filter></defs>
        {edges}{nodes}
        <g filter="url(#gh2)"><circle cx="{hubx}" cy="{huby}" r="82" fill="url(#hub2)"/></g>
        <text x="{hubx}" y="{huby-8}" text-anchor="middle" font-family="DM Sans" font-weight="900" font-size="22" fill="#1a0f0a">BRIEF</text>
        <text x="{hubx}" y="{huby+16}" text-anchor="middle" font-family="DM Mono" font-size="12" fill="#3a2010">ranked, top 1%</text>
        <text x="{hubx}" y="{huby+38}" text-anchor="middle" font-family="DM Mono" font-size="11" fill="#3a2010">1 page</text>
      </svg>
      {cap("people, companies and markets scored before your first call - not a link list.")}</div>'''

# 3. SIGNALS - radar of live signals + a first-to-see lead readout (dark)
def signals():
    cx,cy,R=210,220,190
...
# 4. VOICE - IVORY: style-match ring + a sample written in your voice (light)
def voice():
    ...
    return f'''<div style="width:900px;{CARDIV};padding:36px 42px 34px">
      <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:18px">
        ...
        <span style="font-family:'DM Mono';font-size:13px;letter-spacing:.14em;color:#96562d">PULSE</span></div>
...
# 5. HANDS - isometric build -> test -> ship pipeline ending in a merged PR (dark)
def hands():
...
# 6. MEMORY - radial shared core, 5 agent lifelines, one severed (dark)
def memory():
    cx,cy=210,210
    organs=[("cortex",-90),("specter",-18),("striker",54),("pulse",126),("sentinel",198)]
    lines=""; nodes=""
    ...
    return f'''<div style="width:900px;{CARD};padding:34px 40px 34px;display:flex;align-items:center;gap:20px">
      <svg width="420" height="420" viewBox="0 0 420 420">
        ...
        <filter id="hg3" x="-80%" y="-80%" width="260%" height="260%"><feDropShadow dx="0" dy="0" stdDeviation="18" flood-color="rgb({ACC})" flood-opacity="0.5"/></filter></defs>
        {lines}<g filter="url(#hg3)"><circle cx="{cx}" cy="{cy}" r="52" fill="url(#hb)"/></g>
        <text x="{cx}" y="{cy+5}" text-anchor="middle" font-family="DM Sans" font-weight="900" font-size="17" fill="#1a0f0a">CORE</text></svg>
      <div style="flex:1">
        {htitle("Memory is the core","SHARED MEMORY")}
        <div style="font-family:DM Sans;font-size:19px;color:#c9c3b8;line-height:1.45">ICP, pipeline, pricing, docs - every agent draws from one memory. Cut it and the stack goes dark.</div>
        {cap("one core feeds them all. no memory, no operator.")}</div></div>'''

# 7. GATE - full-capability orb held on the operator's reins, one lock (dark)
def gate():
...
# 8. PRICE - IVORY invoice compare: the stack you replace ($$$) vs Ultron cents (light, NEW form)
def price():
    rows=[("Research tool","$99"),("Outbound sender","$149"),("SDR seat","$1,500"),("Ops platform","$99")]
    lines=""
    for nm,val in rows:
        lines+=(f'<div style="display:flex;align-items:baseline;justify-content:space-between;padding:11px 0;border-bottom:1px solid rgba(150,120,80,.16)">'
          f'<span style="font-family:DM Sans;font-size:18px;color:#5a4634">{nm}</span>'
          f'<span style="font-family:DM Mono;font-size:18px;color:#8a745a">{val}</span></div>')
    left=(f'<div style="flex:1;background:rgba(255,255,255,.5);border:1px solid rgba(150,120,80,.2);border-radius:18px;padding:20px 22px">'
      f'<div style="font-family:DM Mono;font-size:13px;letter-spacing:.12em;color:#8a745a;margin-bottom:8px">THE STACK YOU REPLACE</div>'
      f'{lines}'
      f'<div style="display:flex;align-items:baseline;justify-content:space-between;margin-top:14px">'
      f'<span style="font-family:DM Sans;font-weight:800;font-size:18px;color:#2a2016">Every month</span>'
      f'<span style="font-family:DM Sans;font-weight:900;font-size:34px;color:rgb(200,70,35)">$1,847</span></div></div>')
    right=(f'<div style="flex:1;background:linear-gradient(160deg,#f6e7d7,#eed6bf);border:1.5px solid #96562d;border-radius:18px;padding:24px 22px;display:flex;flex-direction:column;justify-content:center;text-align:center;box-shadow:0 18px 34px rgba(150,90,45,.18)">'
      f'<div style="font-family:DM Mono;font-size:13px;letter-spacing:.12em;color:#96562d;margin-bottom:10px">ULTRON, PER TASK</div>'
      f'<div style="font-family:DM Sans;font-weight:900;font-size:72px;color:#2a2016;line-height:.9">0.11<span style="font-size:38px">c</span></div>'
      f'<div style="font-family:DM Sans;font-size:17px;color:#5a4634;margin-top:10px">pay per token</div>'
      f'<div style="font-family:DM Sans;font-size:17px;color:#5a4634">no seats, no minimum</div></div>')
    return f'''<div style="width:900px;{CARDIV};padding:34px 40px 34px">
      {htitle("A full team for cents a task","PRICE, HONEST","#2a2016")}
      <div style="display:flex;align-items:stretch;gap:20px">
        {left}
        <div style="display:flex;align-items:center;font-family:DM Mono;font-size:16px;color:#8a745a">vs</div>
        {right}
      </div>
      {cap("the stack bills thousands a month. the operator bills cents a task.","#8a745a")}</div>'''

PANELS={"router":router(),"research":research(),"signals":signals(),"voice":voice(),
        "hands":hands(),"memory":memory(),"gate":gate(),"price":price()}
if __name__=="__main__":
    outd=f"{ROOT}/content/_hitl-src/models_clay/s2src27"; os.makedirs(outd,exist_ok=True)
