#!/usr/bin/env python3
# Push an adapted "IG Scraped 2" material: UPDATE the existing source row with the IG deck (1920),
# and INSERT a paired TikTok 3D row (1350). Both tagged ["IG Scraped 2"]. Title read from build_{slug}.py.
...
def upload(slug,kind,bust):
    ...
    for i,f in enumerate(files,1):
        k=f"imports/pm/{slug}-s2{kind}-{bust}/{i:02d}.png"; r2put(k,f); keys.append(k)
...
d1("INSERT INTO vault_items (id,owner,kind,name,source,media,thumb_key,tags,created_at,caption) VALUES (?,?,?,?,?,?,?,?,?,?)",
   [vid,OWNER,"carousel",f"Review · {title} (TikTok 3D)","ig-scraped-2",tt_media,tt_keys[0],'["IG Scraped 2"]',ca-1,""])
