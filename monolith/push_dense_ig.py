#!/usr/bin/env python3
# Replace an IG Scraped row's FULL media with the rebuilt dense _ig deck (new cachebust keys).
# Slide 1 stays the transparent cover. Usage: python3 push_dense_ig.py <slug> <NAME substring>
...
files=sorted(glob.glob(f"{ROOT}/scratchpad/{slug}_ig/s*.png"), key=lambda f:int("".join(c for c in os.path.basename(f) if c.isdigit())))
...
rows=d1(f"SELECT id,name FROM vault_items WHERE owner='{OWNER}' AND name LIKE '%{esc(namesub)}%' AND name LIKE '%Instagram%'")
...
print(f"{slug}: IG row '{rows[0]['name']}' -> {len(keys)} dense slides, thumb={keys[0]}")
