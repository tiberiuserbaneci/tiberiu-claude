...
**TIKTOK ENGAGEMENT MECHANIC (operator, 2026-06-30 — binding, per §0.1):** TikTok is NOT Instagram.
On IG the keyword-DM CTA works (users are ManyChat-trained, DMs deliver). On TikTok the same CTA
...
- **3D object framing - show the COMPLETE element, scaled, NEVER trimmed (operator HARD rule, 2026-06-27, flagged 3x):** the Vertex objects are generated WHOLE and verified - any cut is a framing bug on our side, not the object. Frame each object by its DIFFERENCE FROM ITS OWN CORNER BACKGROUND (the most-inclusive bbox that is still the panel, stopping before the soft vignette), NOT a tight bright-pixel/edge box that slices the soft right/bottom edge. Then SCALE the panel to fill the container and CENTRE THE PANEL (not the crop, whose shadow margins are uneven) at the canvas X-centre and the SAME fixed Y on every slide, so no element sits higher or lower than another. Do for every page what the working pages do. If a panel looks cut, the crop is wrong - fix the crop; never ship a trimmed element.
